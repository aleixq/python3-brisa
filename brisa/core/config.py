...
class ConfigurationManager(object):
    ...
    def update(self):
        ...
        data.
        """
        self._state = pickle.load(open(self._config_path, 'rb'))

    def save(self):
        """ Stores the state of the manager on the persistence.
        """
        pickle.dump(self._state, open(self._config_path, 'wb'),
...
_default_config = {'brisa':
                        {'owner': 'brisa',
                         ...
                         'event_reload_time': '30',
                         'force_event_reload': 'False'}}
...
class _BRisaConfigurationManager(ConfigurationManager):
    ...
    def _check_config_file(self):
        ...
        if os.path.exists(brisa_home):
            ...
            # Home does not exist
            os.mkdir(brisa_home)
            os.chmod(brisa_home, 0o777)
            self._merge_configurations = False
            self._initialize_database()
            os.chmod(self._config_path, 0o666)
